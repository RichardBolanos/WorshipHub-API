    ...
    fun handleGoogleLogin(email: String, fullName: String, givenName: String, familyName: String): OAuth2LoginResult {
        return try {
            ...
            if (existingUser != null) {
                // Activate and verify user if logging in with OAuth2
                val updatedUser = if (!existingUser.isActive || !existingUser.isEmailVerified) {
                    val activated = existingUser.copy(
                        isActive = true,
                ...
                val token = jwtTokenService.generateToken(
                    ...
                    updatedUser.churchId.toString(), 
                    listOf(updatedUser.role.name)
                )
