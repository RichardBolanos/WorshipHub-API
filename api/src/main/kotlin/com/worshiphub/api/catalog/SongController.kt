...
class SongController(
    ...
    @ApiResponses(value = [
        ApiResponse(responseCode = "201", description = "Song successfully created", 
                   content = [Content(schema = Schema(implementation = CreateSongResponse::class))]),
        ApiResponse(responseCode = "400", description = "Invalid song data - missing required fields or invalid format"),
        ApiResponse(responseCode = "404", description = "Church not found with provided ID"),
        ApiResponse(responseCode = "403", description = "User lacks WORSHIP_LEADER or CHURCH_ADMIN role"),
        ApiResponse(responseCode = "409", description = "Song with same title already exists in church catalog")
    ])
    @PostMapping
    ...
    fun createSong(
        ...
        return if (result.isSuccess) {
            CreateSongResponse(songId = result.getOrThrow())
        } else {
            throw BadRequestException(result.exceptionOrNull()?.message ?: "Failed to create song")
        }
    }
    ...
    fun transposeSong(
        ...
        return if (result.isSuccess) {
            ...
            throw BadRequestException(result.exceptionOrNull()?.message ?: "Failed to transpose song")
        }
    }
    ...
    fun getAllSongs(
        ...
        val result = catalogApplicationService.getAllSongs(churchId)
        val songs = if (result.isSuccess) {
            result.getOrThrow()
        } else {
            throw NotFoundException(result.exceptionOrNull()?.message ?: "Failed to get songs")
        }
    ...
    fun searchSongs(
        ...
        val result = catalogApplicationService.searchSongs(query, churchId)
        val songs = if (result.isSuccess) {
            result.getOrThrow()
        } else {
            throw NotFoundException(result.exceptionOrNull()?.message ?: "Failed to search songs")
        }
        val content = songs.map { song ->
            SongResponse(
                id = song.id,
    ...
    @ApiResponses(value = [
        ApiResponse(responseCode = "200", description = "Song successfully updated"),
    ...
    fun updateSong(
        ...
        return if (result.isSuccess) {
            ...
            throw BadRequestException(result.exceptionOrNull()?.message ?: "Failed to update song")
        }
    }
    ...
    fun deleteSong(@PathVariable id: UUID) {
        ...
        if (result.isFailure) {
            throw NotFoundException(result.exceptionOrNull()?.message ?: "Failed to delete song")
        }
    }
}