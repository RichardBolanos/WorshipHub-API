...
class PasswordManagementController(
    ...
    @GetMapping("/debug")
    fun debugAuth(request: jakarta.servlet.http.HttpServletRequest): ResponseEntity<Map<String, Any>> {
        ...
        return ResponseEntity.ok(mapOf(
            "authHeader" to (authHeader ?: "MISSING"),
            "principal" to (principal?.toString() ?: "ANONYMOUS"),
            "authenticated" to (org.springframework.security.core.context.SecurityContextHolder.getContext().authentication?.isAuthenticated ?: false)
        ))
    }
